---
title: "<br>Trading based on MA and EMA directions"
author: "Mark Ziemann"
date: "`r Sys.Date()`"
output:
    rmdformats::downcute:
...
price2 <- price
price2$ma <- ma
price2$higher <- c(NA,sapply(2:length(ma), function (x) { ma[x] > ma[x-1] } ))
...
price2 <- price
price2$ma <- ma
price2$higher <- price2$higher <- c(NA,sapply(2:length(ma), function (x) { ma[x] > ma[x-1] } ))
yesterday <- price2$higher
yesterday <- c(NA,yesterday)
yesterday <- yesterday[1:(length(yesterday)-1)]
price2$yesterday <- yesterday
