---
title: "<br>Trading based on MA and WMA crosses"
author: "Mark Ziemann"
date: "`r Sys.Date()`"
---
...
res_wma <- c(0,0,sapply(3:200,function(n) { wma(price,n,fee) } ) )
...
plot(res_wma,pch=19,xlab="moving average interval (days)",ylab="fold return",main="WMA")
abline(h=hodl,col="red")
grid()

plot(res_wma,pch=19,xlim=c(1,50),xlab="moving average interval (days)",ylab="fold return",main="WMA")
abline(h=hodl,col="red")
grid()
...
```{r,best_wma,fig.width=10,fig.height=8}

best_wma = which(res_wma==max(res_wma))
best_wma
ma <- WMA(Cl(price),n=best_wma)
price2 <- price
price2$ma <- ma
price2$higher <- price2$Close > price2$ma
yesterday <- price2$higher
...
wma_trades <- cbind(buy,sell)
wma_trades$diff <- ( ( wma_trades$sell_price - wma_trades$buy_price ) / wma_trades$buy_price ) +1 - ( 2 * fee )
wma_trades
nrow(wma_trades)
prod(wma_trades$diff)
WMA_HEADER=paste(best_wma,"day weighted moving average")

plot(price$Close~as.Date(price$date),type="l",log="y",
  xlab="Date",ylab="price (USD)",main=WMA_HEADER)
grid()
lines(ma~ as.Date(price$date) ,col="red")
...
res_wma_fees <- sapply(fee_levels,function(x) { wma(price,best_wma,fee=x) } )
plot(fee_levels,res_wma_fees ,pch=19,xlab="fee proportion",ylab="fold return",main="Effect of fees on returns")
mtext(WMA_HEADER)
abline(h=hodl,col="red")
grid()
...
wma_trades$year <- sapply(strsplit(wma_trades[,3],"-"),"[[",1)
wma_yearly <- sapply(years,function(y) { x <- subset(wma_trades,year==y)[,5] } )
vioplot(wma_yearly)
barplot(sapply(wma_yearly,prod))
abline(h=1,col="red",lty=2)
