---
title: "Trading ETH based on the Stochastic indicator"
...
Here, we're using the stochastic as a buy/sell indicator for trading ETH.
...
In the second chart, I'm looking at the strategies with <200 trades since 2015.
Any strategy with mean ROI > 1.15 is considered good. 
...
Here's the table of results for the strategies with ROI of >10,000X and mean trade ROI of 115%.

```{r,res4}

res5 <- subset(res2,returns > 1E5 & mean > 1.15 )
res5 <- res5[order(-res5$returns),]
head(res5,20) %>% kbl() %>%  kable_paper("hover", full_width = F)

```

Now that I know which ranges of values work well, I will zoom in and do a more comprehensive
backtesting for the following values nfastk(12:47),nfastd/nslowd(12:72) and smooth (1:5).
...
res3 <- subset(res2,trades<200 & mean>1.15)
...
This one looks to be the best: nFastK=46, nSlowD/nFastD=25, smooth=1
...
The stochastic gave some nice returns with ROI 26,000x over hodl, which is better than the 34D moving 
average (18x), DMI (74x) and TSI (822x).
