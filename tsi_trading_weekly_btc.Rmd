---
title: "Trading BTC based on the weekly True Strength Index (TSI)"
...
Here we can see the TSI over the past 100 weeks, using the default settings.
...
All trades are executed at the end of the weekly session (UTC midnight).
...
Now let's go back in time to test the performance of the different interval strategies on the weekly
timeframe, here are the ranges tested:

* n1: from 5 to 20 weeks

* n2: from 2 to 10 weeks

* n_signal: from 2 to 5 weeks
...
Here I rank the strategies by the total returns and then rank by the mean ROI, then calculate the rank 
sum.

Here are the top results.

```{r,ranksum,fig.height=8,fig.width=8}

res4$meanroirank <- rank(res4$mean)
res4$returnsrank <- rank(res4$returns)
res4$ranksum <- res4$meanroirank + res4$returnsrank
res4 <- res4[order(-res4$ranksum),]
head(res4) %>% kbl() %>%  kable_paper("hover", full_width = F)
