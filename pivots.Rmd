---
title: "<br>Pivot points for BTC, ETH and XMR"
date: "`r Sys.Date()`"
output:
    fig_width: 15
    fig_height: 12
...
[Source code](https://github.com/MDZanalytics/cryptoblog/blob/main/pivots.Rmd)
...
The pivot also has pivot top (PY) and pivot bottom (PB) lines.
...
x <- lapply(pivml, function(x) {
  ...
  lines(c(as.Date(x[[1]]),as.Date(x[[2]]) ), c( x[13] , x[13] ) , lty=2, col="black") #pb
  lines(c(as.Date(x[[1]]),as.Date(x[[2]]) ), c( x[14] , x[14] ) , lty=2,col="black") #pt
} )
...
tail(piv) %>% kbl(row.names=FALSE, caption="Recent price action and weekly pivots") %>%  kable_styling()
...
tail(piv) %>% kbl(row.names=FALSE, caption="Recent price action and monthly pivots") %>%  kable_styling()
...
tail(piv) %>% kbl(row.names=FALSE, caption="Recent price action and quarterly pivots") %>%  kable_styling()
...
tail(piv) %>% kbl(row.names=FALSE, caption="Recent price action and yearly pivots") %>%  kable_styling()
...
Obtaining XMR historical data (daily) from CoinMarketCap.com from 2014 to present.
...
## Quarterly Pivot points for XMR
