---
title: "Trading BTC based on True Strength Index (TSI)"
...
mytsi <- function(price,n1,n2,ns,fee,  offset=0.5 , multiplier=1) {
...
basic_returns <- basic[2]
basic_mean <- basic[2]^(1/basic[3])
...
head(res2[order(-res2$mean),],20) %>% kbl(caption="Highest mean ROI TSI strategy (unadjusted)") %>%  kable_paper("hover", full_width = F)

head(res2[order(-res2$mean_adjusted),],20) %>% kbl(caption="Highest mean ROI TSI strategy (adjusted)") %>%  kable_paper("hover", full_width = F)
...
head(res2[order(-res2$returns),],20) %>% kbl(caption="Highest ROI TSI strategy (unadjusted)") %>%  kable_paper("hover", full_width = F)

head(res2[order(-res2$returns_adjusted),],20) %>% kbl(caption="Highest ROI TSI strategy (unadjusted)") %>%  kable_paper("hover", full_width = F)
...
res3 <- subset(res2,mean_adjusted>=1.09)
res3 <- res3[order(-res3$returns),]
head(res3,20) %>% kbl(caption="Highest ROI TSI strategy with mean>1.1 (adjusted)") %>%  kable_paper("hover", full_width = F)
...
plot(res3$mean_adjusted , res3$returns_adjusted, log="y",xlab="mean ROI", ylab="total returns")
points(basic_mean,basic_returns,col="red",pch=19,cex=2)
abline(h=hodl,col="red")
grid()
mtext("adjusted")
...
head(res2[order(-res2$returns),],20) %>% kbl(caption="Highest overall ROI TSI strategy (unadjusted)") %>%  kable_paper("hover", full_width = F)

head(res2[order(-res2$returns_adjusted),],20) %>% kbl(caption="Highest overall ROI TSI strategy (unadjusted)") %>%  kable_paper("hover", full_width = F)
...
When accounting for changing performance over time the realistic hodl returns were 5.9
