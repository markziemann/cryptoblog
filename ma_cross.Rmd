...
res_sma <- sapply(1:nrow(mas),function(n) { smacross(price,n,fee) } )
mas1 <- as.data.frame(mas)
mas1$returns <- res_sma
head(mas1[order(-mas1$returns),],20)

res_ema <- sapply(1:nrow(mas),function(n) { emacross(price,n,fee) } )
mas2 <- as.data.frame(mas)
mas2$returns <- res_ema
