---
title: "Trading ETH based on the double RSI"
...
There are many indicators of market momentum, and the most important of these 
is the relative strength index (RSI).

The RSI is a momentum indicator with a 14-day basis and oscillates between 0
and 100, and >50 indicates an uptrend and <50 as downtrend.
Some traders use the levels of 30 and 70 as oversold and overbought signals for entry,
however we know that bitcoin can remain at these extreme levels for a long time,
which would lead to missing big gains and taking big losses.

Therefore the use of arbitrary levels for entry and exit leads to poor results and
dynamic levels are optimal. 
Spotting changes in the direction of the RSI is crucial.
To do this, I will be using two RSI lines with different intervals, a short term 
line and a longer term line.
This is called the [double RSI strategy](https://www.trendfollowingsystem.com/double-rsi-trading-system/) is 
similar in principle to the stochastic indicator, true strength index and some moving average strategies.
The theory is that when the short term line goes above the long-term line
it is a signal that the market could be turning positive.
Similarly when the short-term line goes below the longer-term line it is a sign that
the market might be turning negative.

Here I will be testing different values for the intervals and find the most profitable combination.

This page is designed to be updated daily.
...
## Basic look at the RSI indicator

Here we can see the RSI over the past 730 days, using the default interval of 14 and 28 days.
...
## RSI2 trading function

Here is the function for trading based on RSI crosses, buying when positive momentum indicator 
...
Now let's go back in time to test the performance of the RSI indicator using different 
...
params <- expand.grid(4:100,10:200)
...
Based on this we can do some filtering for mean>1.2 and returns>1e6.


```{r,filt2}

top <- subset(res,mean>1.2&returns>1e6)
...
This is good indicator because mean gains are >1.1 and number of trades is less than 200.
...
This looks like a promising strategy, and I'm particularly interested in the strategies with mean 
ROI>1.2 and total returns >1e6.

This indicator performed better than the TSI and DMI but not better than the Stoch.
