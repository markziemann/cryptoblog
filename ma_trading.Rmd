...
Now let's look at which of the approaches works best.
...
barplot(yr,ylab="yearly fold returns",main="BTC daily SMA")
...
barplot(yr,ylab="yearly fold returns",main="BTC daily EMA")
text(x=(1:length(yr)*1.2)-0.4,y=yr*0.9,labels=signif(yr,2),cex=1.2)
abline(h=1,col="red",lty=2)
...
Based on minimising the number of trades for only a small reduction of returns, it appears the 43d SMA is preferred.
The fee rate strongly affects the fold return, so it is definitely worthwhile to seek exchanges with lower fees.
Profitability varies strongly by year with bull market most profitable and bear market least profitable, meaning it is probably a 
good idea to sit funds in fiat for 12 months following a cycle top.
