...
Data source: https://au.investing.com/indices/aus-200-historical-data

```{r,getdata}

dat <- read.csv("S_and_P_ASX_200_Historical_Data-3-.csv",sep=",",header=TRUE)
date <- dat$Date
Close <- as.numeric(gsub(",","",dat$Price))
...
Assuming 0.4% trading fee, or $20 per trade of $5k
...
res_sma <- c(0,0,sapply(3:400,function(n) { sma(price,n,fee) } ) )

res_ema <- c(0,0,sapply(3:400,function(n) { ema(price,n,fee) } ) )
