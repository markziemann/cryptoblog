...
Apply a slow moving average to the signal to indicate entry/exits.

```{r,sig_sma1}

sig_sma <- function(price,n,fee) {
ma <- SMA(price$sig,n=n)
price2 <- price
price2$ma <- ma
price2$higher <- price2$sig > price2$ma
yesterday <- price2$higher
yesterday <- c(NA,yesterday)
yesterday <- yesterday[1:(length(yesterday)-1)]
price2$yesterday <- yesterday
...
res <- t(sapply(3:200, function(n) { sig_sma(price2,n,fee) }  ))
res <- as.data.frame(res)
colnames(res) <- c("returns","trades")
rownames(res) <- 3:200
res$mean <- res$returns^(1/res$trades)
...
plot(x=myrange, y=res$returns, pch=19,
  xlab="moving average interval (days)",ylab="fold return",main="SMA")
abline(h=hodl2,col="red")
grid()
...
plot(res$mean,res$returns, main="SMA",
  xlab="mean ROI",ylab="total ROI",cex=2,pch=19,col="gray")
grid()
text(res$mean,res$returns , cex=1, labels=rownames(res_sma))
abline(h=hodl2,col="red")

plot(res$trades,res$returns, main="SMA",
  xlab="no. trades",ylab="total ROI",cex=2,pch=19,col="gray")
grid()
text(res$trades,res$returns , cex=1, labels=rownames(res_sma))
abline(h=hodl2,col="red")
...
res_ema$ln <- log(res_ema$mean)
price2 <- price[200:nrow(price),]
mas <- sapply(3:200,function(n) {EMA(price$Close,n) } )
mas2 <- mas[200:nrow(mas),]
hodl2 <- price2[nrow(price2),2] / price2[1,2]
...
plot(price2$sig~as.Date(price2$date),type="l",
  xlab="Date",ylab="sig",main="EMA signal")
grid()
...
sig_ema <- function(price,n,fee) {
ma <- EMA(price$sig,n=n)
price2 <- price
price2$ma <- ma
price2$higher <- price2$sig > price2$ma
yesterday <- price2$higher
