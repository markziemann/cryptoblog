...
[Source code](https://github.com/MDZanalytics/cryptoblog/blob/main/ma_trading_btc.qmd)
...
Trading off simple moving averages (SMA), exponenial moving averages (EMA) and
crosses between these is a popular method for timing entry and exit of positions.
In this blog post I will be exploring MA strategies on the daily timeframe to find
the ones which perform best.
...
head(res_sma[order(-res_sma$mean),],20) %>% kbl(caption="Highest ROI SMA strategy",row.names = FALSE) %>%  kable_styling(full_width = F)
...
head(res_ema[order(-res_ema$mean),],20) %>% kbl(caption="Highest ROI EMA strategy",row.names = FALSE) %>%  kable_styling(full_width = F)
...
sma_trades  %>% kbl(row.names = FALSE, caption="SMA strategy trade history") %>%  kable_styling(full_width = F)
...
sma_trades  %>% kbl(row.names = FALSE, caption="EMA strategy trade history") %>%  kable_styling(full_width = F)
...
These SMA and EMA strategies are simple and profitable, but the hit rate is low in
