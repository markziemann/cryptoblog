...
In the backtesting analysis, I have established different profitable strategies for timing entry/exits.
Click on the hyperlinks below to see how these signals were identified.

1. Simple moving averages for [BTC](https://mdz-analytics.com/crypto/ma_trading.html) and
[ETH](https://mdz-analytics.com/crypto/ma_trading_eth.html).

2. Moving average crosses for [BTC](https://mdz-analytics.com/crypto/ma_cross.html) and
[ETH](https://mdz-analytics.com/crypto/ma_cross_eth.html).

3. Directional movement index (DMI) for [BTC](https://mdz-analytics.com/crypto/dmi_trading.html) and
[ETH](https://mdz-analytics.com/crypto/dmi_trading_eth.html).

4. True strength index (TSI) for [BTC](https://mdz-analytics.com/crypto/tsi_trading.html) and
[ETH](https://mdz-analytics.com/crypto/tsi_trading_eth.html).

5. Stochastic oscillator (stoch) for [BTC](https://mdz-analytics.com/crypto/stoch_trading.html) and
[ETH](https://mdz-analytics.com/crypto/stoch_trading_eth.html).

6. Double RSI and [BTC](https://mdz-analytics.com/crypto/rsi2trading.html) and
[ETH](https://mdz-analytics.com/crypto/rsi2_trading_eth.html).
...
if ( btc_signal != "NONE") {
  MYNOTE <- paste(MYNOTE, "The BTC 12/21 day SMA cross signal is", btc_signal)
}
...
if ( btc_signal != "NONE") {
  MYNOTE <- paste(MYNOTE, "BTC is flashing a ", btc_signal)
}
...
nFastk = 16, nFastD = 54, smooth = 6

```{r,btc_stoch,fig.height=10,fig.width=8}

sto <- stoch(HLC(price), nFastK=16 , nFastD=54 , nSlowD=54 , bounded = TRUE, smooth=6)
...
rsi1 <- RSI(price$close,n=57,maType=EMA)
...
if ( eth_signal != "NONE") {
  MYNOTE <- paste(MYNOTE,"The ETH 5/27 day SMA cross signal is", eth_signal)
}
...
if ( eth_signal != "NONE") {
  MYNOTE <- paste(MYNOTE, "ETH is flashing a ", eth_signal)
}
...
tsi <- TSI(price$close , n.first = 97, n.second = 9, n.signal = 19 )
colnames(tsi) <- c("tsi","sig")
