...
Source: https://github.com/markziemann/cryptoblog/blob/main/ma_cross_eth.Rmd
...
mydate <- Sys.Date()-1
URL=paste("https://web-api.coinmarketcap.com/v1/cryptocurrency/ohlcv/historical?symbol=ETH&convert=USD&interval=daily&time_start=2015-01-01&time_end=",mydate,sep="")
...
best_sma %>% kbl(caption="Best ROI SMA strategies") %>%  kable_styling()
...
best_ema %>% kbl(caption="Best ROI EMA strategies") %>%  kable_styling()
...
Best SMA cross strategy (as at last script edit) was 5/27d, with returns slightly lower than the optimal single SMA strategy (34d/32,000x), but this was achieved with only 43
trades as compared to 59 trades with the single SMA strategy.
The Best EMA cross strategy was 13/16d and got returns similar to the single SMA, but with only 
27 trades.
This could be interpreted as a trade signal with fewer false positives.
In conclusion, the EMA cross strategy gave favourable returns and fewer false positive compared 
to the single MA strategy.
In the past few years, ETH this type of indicator has given returns in excess of similar 
indicators for BTC.
