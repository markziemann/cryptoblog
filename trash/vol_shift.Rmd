...
price <- btcdat$data$quotes$quote$USD

price$range <- price$high - price$low

price$rel_range <- price$range / price$open

price2 <- price
n=10
rr_ma <- sapply(n:nrow(price) , function(x) {  mean(price$rel_range[(x-n):x]) })
price2$rr_ma <- c(rep(0,n-1),rr_ma)
price$timestamp <- gsub("T23:59:59.999Z","",price$timestamp)

par(mfrow=c(2,1))
par(mar=c(7,5,2,2))
plot(price2$close,log="y",pch=19,cex=0.5,type="l",xaxt = "n" , xlab="")
axis(1, at=1:nrow(price), labels=price$timestamp,las=2)
grid()

plot(price2$rr_ma,log="y",pch=19,cex=0.5,type="l",xaxt = "n" , xlab="")
axis(1, at=1:nrow(price), labels=price$timestamp,las=2)
grid()
...
myend <- price$close[nrow(price)]
mystart <- price$close[1]
...
res_ema <- c(0,0,sapply(3:200,function(n) { ema(price,n,fee) } ) )
