...
if ( ! file.exists("ethdat_1w.txt") ) {

  download.file("https://web-api.coinmarketcap.com/v1/cryptocurrency/ohlcv/historical?symbol=ETH&convert=USD&interval=weekly&time_start=2013-06-01&time_end=2021-02-26",destfile="ethdat_1w.txt")

}

btcdat <- fromJSON("ethdat_1w.txt")
...
res_sma <- c(rep(0,2),sapply(3:50,function(n) { sma(price,n,fee) } ) )

res_ema <- c(rep(0,2),sapply(3:50,function(n) { ema(price,n,fee) } ) )
...
plot(res_sma,pch=19,xlim=c(1,25),xlab="moving average interval (weeks)",ylab="fold return",main="SMA")
abline(h=hodl,col="red")
grid()
...
plot(res_ema,pch=19,xlim=c(1,25),xlab="moving average interval (weeks)",ylab="fold return",main="EMA")
abline(h=hodl,col="red")
grid()
