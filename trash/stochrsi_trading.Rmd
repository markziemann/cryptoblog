---
title: "<br>Trading BTC based on the Stochastic indicator"
date: "`r Sys.Date()`"
---

[Source code](https://github.com/MDZanalytics/cryptoblog/blob/main/stoch_trading.Rmd)
...
sto <- stoch(RSI(price2$close,n=14), nFastK = 14, nFastD = 3, nSlowD = 3, 
...
plot(sto[,1] ~ as.Date(price2$date),type="l",col="blue",
  xlab="Date",ylab="stochRSI",main="stoch fast")
lines(as.Date(price2$date), sto[,2]  , col="red"  )
grid()

fee=0.005

sto <- stoch(RSI(price$close,n=14), nFastK=14 , nFastD=3 , nSlowD=3 , bounded = TRUE, smooth=1)
price2 <- price
...
price2 <- tail(price,200)
sto <- stoch(RSI(price2$close,n=14), nFastK = 21, nFastD = 14, nSlowD = 21, bounded = TRUE, smooth=7)
...
plot(sto[,1] ~ as.Date(price2$date),type="l",col="blue",
  xlab="Date",ylab="stochRSI",main="stoch slow")
lines(as.Date(price2$date), sto[,2]  , col="red"  )
grid()

par(mfrow=c(1,1))

sto <- stoch(RSI(price$close,n=14), nFastK=21 , nFastD=14 , nSlowD=21 , bounded = TRUE, smooth=7)
price2 <- price
price2 <- cbind(price2,sto)
price2$higher <- price2$fastK > price2$fastD
yesterday <- price2$higher
...
mystoch <- function(price,nrsi=14,nFastK=14,nFastD=3,nSlowD=3,smooth=1,fee) {
  sto <- stoch(RSI(price$close,nrsi), nFastK , nFastD , nSlowD , 
    bounded = TRUE, smooth=smooth)
  price2 <- price
  price2 <- cbind(price2,sto)
  price2$higher <- price2$fastK > price2$fastD
  yesterday <- price2$higher
...
head(res3[order(-res3$returns),],50) %>% kbl() %>%  kable_styling("hover", full_width = F)
...
Here's the table of results for the strategies with ROI of >10,000X and mean trade ROI of 110%.

```{r,res4}

res5 <- subset(res2,returns > 1E5 & mean > 1.10 )
res5 %>% kbl() %>%  kable_styling("hover", full_width = F)
...
head(res3,10) %>% kbl() %>%  kable_styling("hover", full_width = F)

```

## Optimisation

This one looks to be the best: nFastK=43, nSlowD/nFastD=47, smooth=1
...
sto <- stoch(RSI(price$close,n=14), nFastK=best_nFastK , nFastD=best_nFastD , nSlowD=best_nFastD , 
...
The stochastic gave some nice returns with ROI 1000x over hodl, which is better than the 43D moving 
average (3.8x), DMI (24.4x) and TSI (151x).
Of all the momentum indicators I've tested, this one looks superior.
