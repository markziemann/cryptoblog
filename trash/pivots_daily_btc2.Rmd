---
title: "<br>Daily pivot point strategy for BTC"
date: "`r Sys.Date()`"
output:
    fig_width: 15
    fig_height: 12
...
mydate <- Sys.Date()-1
URL=paste("https://web-api.coinmarketcap.com/v1/cryptocurrency/ohlcv/historical?symbol=BTC&convert=USD&time_period=hourly&interval=4h&time_start=2021-01-01&time_end=",mydate,sep="")
...
plot(pivd$datetime,pivd$close,type="l")
...
pivd$buy <- pivd$close < pivd$s1
pivd$sell <- pivd$close > pivd$r1
...
buy <- buy[,c("datetime","close")]
sell <- sell[,c("datetime","close")]

res <- data.frame("buydate"=buy$datetime,"selldate"=sell$datetime,
  "buyprice"=buy$close,"sellprice"=sell$close)
...
pivd$buy <- pivd$close < pivd$s2
pivd$sell <- pivd$close > pivd$r2
pivd$diff <- pivd$buy - pivd$sell
pivd2 <- pivd[which(pivd$diff != 0),]
pivd2 <- pivd2[which(c(FALSE,pivd2[2:nrow(pivd2),"diff"] != pivd2[1:nrow(pivd2)-1,"diff"])),]
...
pivd$buy <- pivd$close > pivd$r1
pivd$sell <- pivd$close < pivd$s1
pivd$diff <- pivd$buy - pivd$sell
pivd2 <- pivd[which(pivd$diff != 0),]
pivd2 <- pivd2[which(c(FALSE,pivd2[2:nrow(pivd2),"diff"] != pivd2[1:nrow(pivd2)-1,"diff"])),]
...
pivd$buy <- pivd$close > pivd$r2
pivd$sell <- pivd$close < pivd$s2
pivd$diff <- pivd$buy - pivd$sell
pivd2 <- pivd[which(pivd$diff != 0),]
pivd2 <- pivd2[which(c(FALSE,pivd2[2:nrow(pivd2),"diff"] != pivd2[1:nrow(pivd2)-1,"diff"])),]
