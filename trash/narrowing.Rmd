...
Have a look at the 43 day MA.

```{r,best_sma43,fig.width=10,fig.height=8}
...
sma_trades <- cbind(buy,sell)
nrow(sma_trades)
sma_trades$diff <- ( ( sma_trades$sell_price - sma_trades$buy_price ) / sma_trades$buy_price ) +1 - (2 *>
sma_trades
prod(sma_trades$diff)
SMA_HEADER=paste(best_sma,"day moving average")
...
Look at the 15 day EMA.

```{r,best_ema15,fig.width=10,fig.height=8}
...
ema_trades <- cbind(buy,sell)
ema_trades$diff <- ( ( ema_trades$sell_price - ema_trades$buy_price ) / ema_trades$buy_price ) +1 - ( 2 >
ema_trades
nrow(ema_trades)
prod(ema_trades$diff)
EMA_HEADER=paste(best_ema,"day exponential moving average")
