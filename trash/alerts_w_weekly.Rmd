...
3. Directional movement index (DMI): [BTC](https://mdz-analytics.com/crypto/dmi_trading.html) 32 day;
[ETH](https://mdz-analytics.com/crypto/dmi_trading_eth.html) 16 day.

4. True strength index (TSI): [BTC](https://mdz-analytics.com/crypto/tsi_trading.html) n1=104, n2=15, n.signal=5;
[ETH](https://mdz-analytics.com/crypto/tsi_trading_eth.html) n1=101, n2=7, n.signal=20.

5. Stochastic oscillator (stoch): [BTC](https://mdz-analytics.com/crypto/stoch_trading.html) nFastK=7, nFastD=45, smooth=17;
[ETH](https://mdz-analytics.com/crypto/stoch_trading_eth.html) nFastK=20, nFastD=7, smooth=36.

6. Double RSI: [BTC](https://mdz-analytics.com/crypto/rsi2trading.html) n1=43, n2=122;
[ETH](https://mdz-analytics.com/crypto/rsi2_trading_eth.html) n1=103, n2=107.
...
DMI using a 32 day interval gives returns similar to than the 43D SMA strategy.

```{r,btc_dmi,fig.height=10,fig.width=8}

dmi.adx <- ADX(price[,c("high","low","close")],n=32)
...
nFastk = 7, nFastD = 45, smooth = 17

```{r,btc_stoch,fig.height=10,fig.width=8}

sto <- stoch(HLC(price), nFastK=7 , nFastD=45 , nSlowD=45 , bounded = TRUE, smooth=17)
...
rsi1 <- RSI(price$close,n=43,maType=EMA)
rsi2 <- RSI(price$close,n=122,maType=EMA)
...
## Get weekly BTC price data and look at the weekly SMA

Obtaining BTC price data (weekly) for the last 100 weeks.

This should only run on Sunday night just before the weekly close.

The 6 week moving average is the most important for us.
...
if (weekdays(Sys.Date() == "Sunday" ) {
  ...
  mydate <- Sys.Date()
  URL=paste("https://web-api.coinmarketcap.com/v1/cryptocurrency/ohlcv/historical?symbol=BTC&convert=USD&interval=weekly&time_start=",datebefore,"&time_end=",Sys.Date(),sep="")
  download.file(URL,destfile="btcdat.txt")
  btcdat <- fromJSON("btcdat.txt")
  price <- btcdat$data$quotes
  price <- data.frame( as.Date(price$time_close) , price$quote$USD$high, price$quote$USD$low, price$quote$USD$close)
  ...
  if (today_signal==TRUE) {
    system("sed -i 's/BTC_SMA_WEEKLY/BULLISH/' index1.html")
    system("sed -i 's/BTC_SMA_WEEKLY_COL/green/' index1.html")
  } else {
    system("sed -i 's/BTC_SMA_WEEKLY/BEARISH/' index1.html")
    system("sed -i 's/BTC_SMA_WEEKLY_COL/red/' index1.html")
  }

  message(paste("BTC weekly SMA signal:",btc_signal))

} else {

  x <- readLines("https://mdz-analytics.com/index.html")
  x <- x[grep("SMA weekly",x)+1]
...
For ETH, the 16 day interval DMI gave a good return.

```{r,eth_dmi,fig.height=10,fig.width=8}

dmi.adx <- ADX(price[,c("high","low","close")],n=16)
price2 <- cbind(price,dmi.adx)
price2$signal <- dmi.adx[,1] > dmi.adx[,2]
tail(price2) %>% kbl() %>%  kable_styling("hover", full_width = F)
...
n1=101, n2=7, n.sig=20

```{r,eth_tsi,fig.height=10,fig.width=8}

tsi <- TSI(price$close , n.first = 101, n.second = 7, n.signal = 20 )
colnames(tsi) <- c("tsi","sig")
...
nFastk = 20, nFastD = 7, smooth = 36

```{r,eth_stoch,fig.height=10,fig.width=8}

sto <- stoch(HLC(price), nFastK=20 , nFastD=7 , nSlowD=7 , bounded = TRUE, smooth=36)
...
rsi1 <- RSI(price$close,n=103,maType=EMA)
rsi2 <- RSI(price$close,n=107,maType=EMA)
...
## Get weekly ETH price data and look at the weekly EMA

Obtaining ETH price data (weekly) for the last 100 weeks.

This should only run on Sunday night just before the weekly close.

The 5 week exponential moving average is the most important for us.
...
if (weekdays(Sys.Date() == "Sunday" ) {
  datebefore <- Sys.Date()-100
  mydate <- Sys.Date()
  URL=paste("https://web-api.coinmarketcap.com/v1/cryptocurrency/ohlcv/historical?symbol=ETH&convert=USD&interval=weekly&time_start=",datebefore,"&time_end=",Sys.Date(),sep="")
  download.file(URL,destfile="ethdat.txt")
  ethdat <- fromJSON("ethdat.txt")
  price <- ethdat$data$quotes
  price <- data.frame( as.Date(price$time_close) , price$quote$USD$high, price$quote$USD$low, price$quote$USD$close)
  ...
  if (today_signal==TRUE) {
    system("sed -i 's/ETH_SMA_WEEKLY/BULLISH/' index1.html")
    system("sed -i 's/ETH_SMA_WEEKLY_COL/green/' index1.html")
  } else {
    system("sed -i 's/ETH_SMA_WEEKLY/BEARISH/' index1.html")
    system("sed -i 's/ETH_SMA_WEEKLY_COL/red/' index1.html")
  }

  message(paste("ETH weekly EMA signal:",eth_signal))
