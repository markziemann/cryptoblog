...
[Source code](https://github.com/MDZanalytics/cryptoblog/blob/main/stoch_trading_weekly_btc.Rmd)
...
head(res3[order(-res3$returns),],20) %>% kbl() %>%  kable_styling("hover", full_width = F)
...
The stochastic gave 4.6x over hodl, which is similar to the 43D moving 
average and better than the DMI (3.3x) but worse than the TSI (8.5x).
