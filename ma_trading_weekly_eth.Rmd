---
title: "Trading ETH based on weekly MA and EMAs"
...
Source: https://github.com/markziemann/cryptoblog/blob/main/ma_trading_weekly_eth.Rmd
...
Obtaining ETH historical data (weekly) from CoinMarketCap.com from August 2015 to present.
...
Let's look back in time and trial SMA and EMA strategies with an interval from 3 to 20 weeks.
...
plot(x=myrange, y=res_sma$returns, pch=19,
  xlab="moving average interval (weeks)",ylab="fold return",main="ETH weekly SMA")
abline(h=hodl,col="red")
grid()

plot(x=myrange,y=res_ema$returns, pch=19,
  xlab="moving average interval (weeks)",ylab="fold return",main="ETH weekly EMA")
abline(h=hodl,col="red")
grid()
...
We know that bitcoin moves in four year cycles, and that ethereum tends to follow bitcoin price. 
...
vioplot(sma_yearly,ylab="trade ROI distribution",main="ETH weekly SMA strategy")

yr <- sapply(sma_yearly,prod)
barplot(yr,ylab="yearly fold returns",main="ETH weekly SMA strategy")
...
vioplot(ema_yearly,ylab="trade ROI distribution",main="ETH weekly EMA strategy")

yr <- sapply(ema_yearly,prod)
barplot(yr,ylab="yearly fold returns",main="ETH weekly EMA strategy")
text(x=(1:length(yr)*1.2)-0.4,y=yr*0.9,labels=signif(yr,2),cex=1.2)
abline(h=1,col="red",lty=2)
...
The best SMA was on the 5 week interval, and the returns were approx 1.5x better than simply hodling.
The best EMA was also on the 5 wk, and returns were 1.75x better than hodling.
The weekly MA strategy appears to be better with ETH as compared to BTC, simply because ETH is a younger asset and therefore more volatile.
One must be cutious though, because most of the MA intervals tested here gave returns lower than simply hodling.

These weekly MA strategies might be useful if you don't want to be checking charts every day, but the returns shown here were much 
lower than for the ETH daily MA strategy.
