...
if ( nrow(subset(res5,mean>1.1)) == 0 ) {
  res5 <- head(res5[order(-res5$mean),],1)
} else {
  res5 <- subset(res5,mean>1.1)
...
Here are the highest returning strategies with the improved parameters.
...
if ( nrow(subset(res3,mean>1.1)) == 0 ) {
  res3 <- head(res3[order(-res3$mean),],1)
} else {
  res3 <- subset(res3,mean>1.1)
...
Below I show the results for the best strategy, charts and each individual trade.
