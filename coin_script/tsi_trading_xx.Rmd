---
title: "Trading TICKER based on True Strength Index (TSI)"
date: "`r Sys.Date()`"
---
...
The strategy is to discard any strategies with a mean ROI<1.1 and then select the one with the
...
head(res2,20) %>% kbl() %>%  kable_styling("hover", full_width = F)

```

## Filtering part 2

Same filtering approach as above.
...
tsi_dat <- c("indicator"="TSI",
  "parameter"=paste(best_n1,best_n2,best_ns,sep=",")  ,
...
write.table(t(tsi_dat),file=filename,append=TRUE,col.names=FALSE,row.names=FALSE)
