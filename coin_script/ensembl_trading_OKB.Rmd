---
title: "Trading OKB based on indicator ensembles"
author: "Mark Ziemann https://mdz-analytics.com"
date: "`r Sys.Date()`"
---

[Source code](https://github.com/markziemann/cryptoblog/blob/main/coin_script/ensembl_trading_xx.Rmd)
...
URL=paste("https://web-api.coinmarketcap.com/v1/cryptocurrency/ohlcv/historical?symbol=OKB&convert=USD&interval=daily&time_start=2013-06-01&time_end=",mydate,sep="")
...
params <- read.table("OKB_dat.txt", header=TRUE)
...
price2$stoch <- sto[,"fastK"] > sto[,"fastD"]
...
if ( length(which(combo == "SMAcross")) ) {
  ...
  sma1 <- SMA(Cl(price),n=best_sma_cross1)
  sma2 <- SMA(Cl(price),n=best_sma_cross2)
  indicators$SMAcross <- sma2>sma1
}

if ( length(which(combo == "EMA")) ) {
  best_ema = as.numeric(params[which(params$indicator == "EMA"),"parameter"])
  ma <- EMA(Cl(price),n=best_ema)
  indicators$EMA <- price$close > ma
}
...
if ( length(which(combo == "EMAcross")) ) {
  ...
  ema1 <- EMA(Cl(price),n=best_ema_cross1)
  ema2 <- EMA(Cl(price),n=best_ema_cross2)
  indicators$EMAcross <- ema2>ema1
}

if ( length(which(combo == "DMI")) ) {
  n <- as.numeric(params[params$indicator == "DMI",2])
  dmi.adx <- ADX(price[,c("high","low","close")],n=n)
  indicators$DMI <- dmi.adx[,"DIp"]>dmi.adx[,"DIn"]
}
...
if ( length(which(combo == "stoch")) ) {
  ...
  sto <- stoch(HLC(price), nFastK=n1 , nFastD=n2 , nSlowD=n2 ,
    bounded = TRUE, smooth=n3)
  indicators$stoch <- sto[,"fastK"] > sto[,"fastD"]
}
...
filename <- paste("OKB","_dat.txt",sep="")
