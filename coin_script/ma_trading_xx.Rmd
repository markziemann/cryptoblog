...
[Source code](https://github.com/MDZanalytics/cryptoblog/blob/main/ma_trading_xx.Rmd)
...
if ( nrow(subset(res_sma,mean>1.1)) == 0 ) {
  res_sma <- head(res_sma[order(-res_sma$mean),],1)
} else {
  res_sma <- subset(res_sma,mean>1.1)
  res_sma <- res_sma[order(-res_sma$returns),]
}

best_sma = as.numeric(rownames(res_sma)[1])
#as.numeric(rownames(head(res_sma[order(-res_sma$returns),],1)))
...
if ( nrow(subset(res_ema,mean>1.1)) == 0 ) {
  res_ema <- head(res_ema[order(-res_ema$mean),],1)
} else {
  res_ema <- subset(res_ema,mean>1.1)
  res_ema <- res_ema[order(-res_ema$returns),]
}

best_ema = as.numeric(rownames(res_ema)[1])
