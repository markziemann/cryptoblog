---
title: "Trading BTC based on the Aroon indicator"
...
Source: https://github.com/markziemann/cryptoblog/blob/main/aroon_trading.Rmd
...
There are many indicators of market momentum, one of them is the Aroon indicator. 
For this post, I'll be using a simple implementation of the Aroon, by buying when the positive
...
What I am examining here is different intervals for the calculation of the Aroon, typically 
20 day is used, however I will be seeing whether other intervals perform better.
...
## Basic look at the Aroon indicator

Here we can see the Aroon indicator over the past 730 days, using the default interval of 20d.

```{r,aroon1,fig.height=10,fig.width=8}

par(mfrow=c(2,1))
price2 <- tail(price,730)
ar <- aroon(price2[,c("high","low")],n=20)
...
plot(ar[,1] ~ as.Date(price2$date),type="l",col="blue",
  xlab="Date",ylab="index",main="Aroon indicator")
lines(as.Date(price2$date), ar[,2]  , col="red"  )
grid()
...
## Aroon trading function

Here is the function for trading based on Aroon line crosses, buying when positive momentum indicator 
...
arn <- function(price,n,fee) {
  ar <- aroon(price[,c("high","low")],n=n)
  price2 <- price
  price2 <- cbind(price2,ar)
  price2$higher <- price2$aroonUp > price2$aroonDn
...
Now let's go back in time to test the performance of the indicator using different 
...
fee=0.001
myrange <- 4:200
res <- t(sapply(myrange,function(n) { arn(price,n,fee) } ))
...
plot(res$period,res$returns,pch=19,xlab="n interval (days)",ylab="fold return",main="Aroon")
abline(h=hodl,col="red")
grid()
...
The 39 day interval gives a decent return.

```{r,best_sma2,fig.width=10,fig.height=8}

best_n = 39
message(paste("Selected Aroon period:",best_n))
ar <- aroon(price[,c("high","low")],n=n)
price2 <- price
price2 <- cbind(price2,ar)
price2$higher <- price2$aroonUp > price2$aroonDn
...
HEADER=paste(best_n,"Aroon")
par(mfrow=c(2,1))
plot(price$close~as.Date(price$date),type="l",log="y",
  xlab="Date",ylab="price (USD)",main="USD price")
grid()

plot(ar[,1] ~ as.Date(price$date),type="l",col="blue",
  xlab="Date",ylab="Index",main="Aroon",ylim=c(0,60))
lines(as.Date(price$date), ar[,2]  , col="red"  )
grid()
par(mfrow=c(1,1))
...
This indicator gives a very high return, but trades are executed very frequently.
...
res_fees <- sapply(fee_levels,function(x) { arn(price,best_n,fee=x) } )
